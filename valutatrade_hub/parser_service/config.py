...
class ParserConfig:
    ...
    COINGECKO_API_KEY: str = field(default_factory=lambda: os.getenv("COINGECKO_API_KEY", "")) # noqa: E501
    ...
    CRYPTO_CURRENCIES: Tuple[str, ...] = ("BTC", "ETH", "LTC", "XRP", "ADA", "SOL", "DOT") # noqa: E501
    ...
    # Параметры запросов
    REQUEST_TIMEOUT: int = field(default_factory=lambda: settings.get("api_timeout_seconds", 30)) # noqa: E501
    MAX_RETRIES: int = field(default_factory=lambda: settings.get("api_max_retries", 3)) # noqa: E501
    RETRY_DELAY: float = field(default_factory=lambda: settings.get("api_retry_delay", 1.0)) # noqa: E501

    # Интервал обновления (в секундах)
    UPDATE_INTERVAL: int = field(default_factory=lambda: settings.get("rates_update_interval", 300)) # noqa: E501

    # Пути к файлам
    RATES_FILE_PATH: str = field(default_factory=lambda: str(
        Path(settings.get("data_dir", "data")) / "rates.json"
    ))  # noqa: E501
    HISTORY_FILE_PATH: str = field(default_factory=lambda: str(
    ...
    def validate_config(self) -> bool:
        ...
        if not self.EXCHANGERATE_API_KEY or self.EXCHANGERATE_API_KEY == "your_api_key_here": # noqa: E501
            raise ValueError(
                "EXCHANGERATE_API_KEY не настроен. Установите переменную окружения EXCHANGERATE_API_KEY" # noqa: E501
            )  # noqa: E501

        if not all(currency in self.CRYPTO_ID_MAP for currency in self.CRYPTO_CURRENCIES): # noqa: E501
